  ...
  .input(z.object({ movieId : z.string() }))
    .query(async ({ ctx, input }) => {
        ...
        const fetchData = async () => {
            const url = "103.157.97.145:8001/data?" + "movieId=" + input.movieId;
            console.log(url)
    ...
    getRecommendationByCountry: publicProcedure
      .input(z.object({ country : z.string() }))
        ...
        const fetchData = async () => {
            const url = "103.157.97.145:8001/dataByCountry?" + "country=" + input.country;
            console.log(url)
            const response = await fetch(url);
            const data = await response.json();
            return data;
    ...
    getRecommendationByActorGenre: publicProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const fetchData = async () => {
          ...
          const url = `103.157.97.145:8001/dataByActorGenre?${params.toString()}`;
